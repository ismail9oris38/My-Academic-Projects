...
W_GD = np.load("W_GD_intfloat.npy") #(500, 2048)
W_SGD = np.load("W_SGD_intfloat.npy")
W_ADAM = np.load("W_ADAM_intfloat.npy")

y = np.load("reranker_etiketleri.npy")  #(100, 1) # Hedef etiketler (1.0: Doğru, -1.0: Yanlış). Toplam 100 örnek.
X = np.load("test_vektor_intfloat.npy")  #(100, 2048) # Birleştirilmiş girdi gömülmeleri (Sorgu + Cevap). 100 örnek, 2048 boyut.
